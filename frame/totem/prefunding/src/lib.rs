...
mod pallet {
    ...
    use totem_common::types::{accounting::Record as PostingRecord, prefunding::*, ComparisonAmounts, Indicator::*};
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn set_prefunding(
            ...
            let current_balance: ComparisonAmounts = T::PrefundingConversions::convert(T::Currency::free_balance(&s));
            ...
            if current_balance >= minimum_amount {
                let converted_amount: CurrencyBalanceOf<T> = T::PrefundingConversions::convert(c);
        ...
        fn cancel_prefunding_lock(o: T::AccountId, h: T::Hash, s: Status) -> DispatchResultWithPostInfo {
            ...
            // unlock the funds
            T::Currency::totem_remove_lock(prefunding_id, &o).or(Err("Error removing a lock"))?;
    ...
    impl<T: Config> Encumbrance<T::AccountId, T::Hash, T::BlockNumber> for Pallet<T> {
        fn prefunding_for(
            ...
            let currency_amount: CurrencyBalanceOf<T> = T::PrefundingConversions::convert(increase_amount);
            ...
            // manage the deposit
            if let Err(_) = Self::set_prefunding(who.clone(), increase_amount, deadline, prefunding_hash, uid) {
        ...
        fn settle_prefunded_invoice(o: T::AccountId, h: T::Hash, uid: T::Hash) -> DispatchResultWithPostInfo {
            ...
            let (payer, beneficiary) = match Self::get_release_state(h) {
                ...
                (Locked, Locked) => {
                    ...
                    let (prefunded_amount, _) = Self::prefunding(&h).ok_or(Error::<T>::NoPrefunding)?;
                    // convert to Account Balance type
                    let increase_amount: AccountBalanceOf<T> = T::PrefundingConversions::convert(prefunded_amount);
                    let decrease_amount: AccountBalanceOf<T> = {
                        let to_invert: i128 = T::PrefundingConversions::convert(increase_amount);
                        T::PrefundingConversions::convert(-1 * to_invert)
                    };
                    let current_block = frame_system::Pallet::<T>::block_number();
                    let current_block_dupe = frame_system::Pallet::<T>::block_number();
