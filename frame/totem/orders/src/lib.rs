...
mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn create_order(
            ...
            if market_order {
                ...
                if tx_keys_large.record_id == tx_keys_large.parent_id {
                    ...
                    if Self::check_approver(who.clone(), approver, tx_keys_large.record_id) {
                        ...
                        Postulate::<T>::mutate_or_err(&tx_keys_large.parent_id, |v| v.push(tx_keys_large.record_id))?;
            ...
            Self::deposit_event(Event::OrderCreated(tx_keys_large.tx_uid, tx_keys_large.record_id));
        ...
        fn create_spfso(
            ...
            let order_hash = <T as Config>::Accounting::get_pseudo_random_hash(who.clone(), approver.clone());
        ...
        fn handle_spfso(
            ...
            if who == commander {
                ...
                // This is the seller
                //TODO if the order us passed as an arg it doesn't need to be read again
                if let Err(_) = Self::set_state_simple_prefunded_closed_order(who, h, s, order_hdr, tx_uid) {
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn set_simple_prefunded_service_order(
            ...
            if Self::check_approver(commander.clone(), approver.clone(), order_hash) {
                ...
                let deadline_converted = T::OrderConversions::convert(deadline);
                ...
                let balance_amount = T::OrderConversions::convert(amount);
                ...
                Self::set_order(commander, fulfiller, order_hash, order_header, vec_order_items)?;
            } else {
                // the order is not yet approved.
        ...
        fn change_simple_prefunded_order(
            ...
            Self::set_order(order_hdr.commander, fulfiller, reference, order_header, vec_order_items)?;
                ...
                0 => {
                    ...
                    match order_status {
                        ...
                        2 => {
                            ...
                            if let Err(_e) = T::Prefunding::set_release_state(order.commander.clone(), lock, h, uid) {
                                fail!(Error::<T>::InPrefunding3)
                            }
                            // now release the funds lock
                            if let Err(_e) = T::Prefunding::unlock_funds_for_owner(order.commander.clone(), h, uid) {
                ...
                1 => {
                    match order_status {
                        5 => {
                            ...
                            if let Err(_e) =
                                T::Prefunding::send_simple_invoice(f, order.commander.clone(), order.amount, h, uid)
                            {
