...
use frame_support::{decl_event, decl_error, decl_module, decl_storage, dispatch::{DispatchResult}, weights::{Weight, DispatchClass}, StorageValue, StorageMap, traits::Randomness}; // v2
...
decl_storage! {
    trait Store for Module<T: Trait> as AccountingModule {
        ...
        PostingNumber get(fn posting_number): u128;
        // Associate the posting index with the identity
        IdAccountPostingIdList get(fn id_account_posting_id_list): map hasher(blake2_128_concat) (T::AccountId, Account) => Vec<u128>;
        // Convenience list of Accounts used by an identity. Useful for UI read performance
        AccountsById get(fn accounts_by_id): map hasher(blake2_128_concat) T::AccountId => Vec<Account>;
        // Accounting Balances 
        BalanceByLedger get(fn balance_by_ledger): map hasher(blake2_128_concat) (T::AccountId, Account) => LedgerBalance;
        // Detail of the accounting posting (for Audit)
        PostingDetail get(fn posting_detail): map hasher(blake2_128_concat) (T::AccountId, Account, u128) => Option<(T::BlockNumber,LedgerBalance,Indicator,T::Hash, T::BlockNumber)>;
        // yay! Totem!
        GlobalLedger get(fn global_ledger): map hasher(blake2_128_concat) Account => LedgerBalance;
        // Address to book the sales tax to and the tax jurisdiction (Experimental, may be deprecated in future) 
        TaxesByJurisdiction get(fn taxes_by_jurisdiction): map hasher(blake2_128_concat) (T::AccountId, T::AccountId) => LedgerBalance;
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        // fn opening_balance() -> DispatchResult {
        //     Ok(())
        // }
        // fn adjustment() -> DispatchResult {
        //     Ok(())
        // }
    }
}
...
impl<T: Trait> Module<T> {
    ...
    fn post_amounts((o, a, c, d, h, b, t): (T::AccountId, Account, LedgerBalance, bool, T::Hash, T::BlockNumber, T::BlockNumber)) -> DispatchResult {
...
impl<T: Trait> Posting<T::AccountId,T::Hash,T::BlockNumber> for Module<T> {
    ...
    fn handle_multiposting_amounts(
        // o: T::AccountId, No longer required
        ...
        trk: Vec<(T::AccountId, Account, LedgerBalance, bool, T::Hash, T::BlockNumber, T::BlockNumber)>) -> DispatchResult {
        ...
        fn get_pseudo_random_hash(sender: T::AccountId, recipient: T::AccountId) -> T::Hash {
            ...
            let phrase = b"totem_global_functional_currency";
    ...
    decl_error! {
        pub enum Error for Module<T: Trait> {
            /// Overflow error on a single ledger account
            ErrorOverflow,
            /// Overflow error on the global ledger account
            ErrorGlobalOverflow,
            /// Error when trying to reverse another error
            ErrorInError,
        }
    }
